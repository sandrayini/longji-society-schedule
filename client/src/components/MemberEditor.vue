  ...
  <div class="modal-overlay" @click.self="$emit('close')">
    <div class="modal-card">
      ...
      <div class="list">
        <div v-for="m in members" :key="m.id" class="member-row">
          ...
          <div class="info">
            ...
            <div class="contact">{{ m.role || '成员' }} · {{ m.contact || '无联系方式' }}</div>
          </div>
          <div class="actions">
      ...
      <div v-if="editing" class="editor">
        ...
        <div class="form-group"><label class="form-label">姓名</label><input v-model="editing.name" class="input" /></div>
        <div class="form-group"><label class="form-label">职务</label><input v-model="editing.role" class="input" placeholder="如组长/副组长" /></div>
        <div class="form-group"><label class="form-label">联系方式</label><input v-model="editing.contact" class="input" placeholder="手机号或微信号" /></div>
        <div class="form-group"><label class="form-label">登录账号</label><input v-model="editing.username" class="input" :disabled="!!editing.id" /></div>
        <div class="form-group" v-if="!editing.id"><label class="form-label">初始密码</label><input v-model="editing.password" class="input" placeholder="默认 longji123" /></div>
        <div class="form-group"><label class="form-label">状态</label>
          <select v-model="editing.active" class="input">
...
function add() {
  editing.value = { name: '', role: '', contact: '', username: '', password: 'longji123', active: 1 };
}
